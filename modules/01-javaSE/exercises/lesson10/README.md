...
> 配套笔记：`modules/01-javaSE/notes/10-网络编程.md`
> 本目录包名：`lesson10`（参考答案在子包 `lesson10.solutions`）
...
``powershell
cd modules\01-javaSE\exercises\lesson10
javac -encoding UTF-8 -d out Exercise01.java
java -cp out lesson10.Exercise01
``
...
java -cp out lesson10.solutions.Solution01
``

注意：

- 必须加 `-encoding UTF-8`（源文件里有中文注释）
- 加包后，运行命令要用"包名.类名"：`lesson10.Exercise01`
