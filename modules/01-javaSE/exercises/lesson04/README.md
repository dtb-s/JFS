...
> 配套笔记：`modules/01-javaSE/notes/04-集合.md`
> 本目录包名：`lesson04`（参考答案在子包 `lesson04.solutions`）
...
``powershell
cd modules\01-javaSE\exercises\lesson04
javac -encoding UTF-8 -d out Exercise01.java
java -cp out lesson04.Exercise01
``
...
java -cp out lesson04.solutions.Solution01
``

注意：

- 必须加 `-encoding UTF-8`（源文件里有中文注释）
- 加包后，运行命令要用"包名.类名"：`lesson04.Exercise01`
